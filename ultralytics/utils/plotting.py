...
class Annotator:
    ...
    def __init__(self, im, line_width=None, font_size=None, font="Arial.ttf", pil=False, example="abc"):
        ...
        self.lw = line_width or max(round(sum(im.size if input_is_pil else im.shape) / 2 * 0.001), 1)
        if self.pil:  # use PIL
            self.im = im if input_is_pil else Image.fromarray(im)
            self.draw = ImageDraw.Draw(self.im)
            try:
                font = check_font("Arial.Unicode.ttf" if non_ascii else font)
                size = font_size or max(round(sum(self.im.size) / 2 * 0.025), 10)
            ...
            #self.sf = 0.4  # font scale
            self.sf = self.lw / 6  # font scale
        # Pose
        self.skeleton = [
            [16, 14],
            [14, 12],
    ...
    def draw_region(self, reg_pts=None, color=(0, 255, 0), thickness=1):
        """
        Draw region line.
    ...
    def draw_centroid_and_tracks(self, track, color=(255, 0, 255), track_thickness=1):
        """
    ...
    def count_labels(self, counts=0, count_txt_size=1, color=(255, 255, 255), txt_color=(0, 0, 0)):
        """
    ...
    def plot_angle_and_count_and_stage(self, angle_text, count_text, stage_text, center_kpt, line_thickness=1):
        """
    ...
    def visioneye(self, box, center_point, color=(235, 219, 11), pin_color=(255, 0, 255), thickness=1, pins_radius=10):
        """
...
def plot_images(
    ...
    annotator = Annotator(mosaic, line_width=round(fs / 14), font_size=10, pil=True, example=names)
    for i in range(bs):
        x, y = int(w * (i // ns)), int(h * (i % ns))  # block origin
